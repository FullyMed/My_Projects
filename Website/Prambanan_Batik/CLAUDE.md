...
Always use `password_hash()` / `password_verify()` (bcrypt). Never use SHA2.
...
All admin pages include `admin/auth.php` at the top and call `requireAdminLogin()`, which redirects to `/admin/login.php` if the session is invalid. Session state is stored under the key `product_hub_session` with a **30-minute idle timeout** (sliding window — refreshed on every authenticated request via `isAdminLoggedIn()`).
...
- `validateCsrfToken($token)` — call at the top of every POST handler
...
| `sitemap.php` | Generates XML sitemap dynamically from DB; gracefully omits product URLs if DB is null |
...
- When adding new admin pages, include `admin/auth.php` and call `requireAdminLogin()` at the top.
