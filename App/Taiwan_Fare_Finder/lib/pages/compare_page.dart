...
class _ComparePageState extends State<ComparePage> {
  ...
  final Set<TransportMode> _modes = {
    TransportMode.hsr,
    TransportMode.tra,
    TransportMode.mrt
  };
  ...
  Widget build(BuildContext context) {
    ...
    final canCompare = _origin != null &&
        _destination != null &&
        _origin!.id != _destination!.id &&
        _modes.isNotEmpty;
    final recentLocations = _recentFromHistory(history.history);

    final sorted = [...fare.results]
      ..sort((a, b) => _compareResults(a, b, _sort));
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 10,
                  children: [
                    _multiChip(context, TransportMode.hsr, l10n.modesHSR,
                        enabled: !fare.isLoading),
                    _multiChip(context, TransportMode.tra, l10n.modesTRA,
                        enabled: !fare.isLoading),
                    _multiChip(context, TransportMode.mrt, l10n.modesMRT,
                        enabled: !fare.isLoading),
                    _multiChip(context, TransportMode.bus, l10n.modesBus,
                        enabled: !fare.isLoading),
                    _multiChip(
                        context, TransportMode.youBike, l10n.modesYouBike,
                        enabled: !fare.isLoading),
                  ],
                ),
                const SizedBox(height: AppSpacing.md),
  ...
  Widget _multiChip(BuildContext context, TransportMode mode, String label,
...
int _compareResults(FareResult a, FareResult b, CompareSort sort) {
  ...
  switch (sort) {
    ...
    case CompareSort.fewestTransfers:
      cmp = _transferCount(a.transferSummary)
          .compareTo(_transferCount(b.transferSummary));
      if (cmp != 0) return cmp;
      cmp = a.fares.adult.compareTo(b.fares.adult);
      if (cmp != 0) return cmp;
      cmp = a.durationMinutes.compareTo(b.durationMinutes);
