          ...
          Padding(
            ...
            child: Container(
              ...
              child: TabBar(
                ...
                indicator: BoxDecoration(color: cs.primaryContainer.withValues(alpha: 0.85), borderRadius: BorderRadius.circular(999)),
                labelColor: cs.onPrimaryContainer,
                unselectedLabelColor: cs.onSurfaceVariant,
                tabs: [Tab(text: l10n.favorites), Tab(text: l10n.history)],
              ),
            ),
          ),
          Expanded(
            child: TabBarView(
              ...
              children: [
                fav.isLoading
                    ...
                    : (fav.favorites.isEmpty
                        ? TffEmptyState(title: l10n.emptyFavorites, body: l10n.emptyFavoritesBody, icon: Icons.bookmark_border_rounded)
                        : _FavoritesList(items: fav.favorites, offline: settings.offlineMode, dataMode: settings.dataMode)),
                hist.isLoading
                    ? const Center(child: CircularProgressIndicator())
                    : (hist.history.isEmpty
                        ? TffEmptyState(title: l10n.emptyHistory, body: l10n.emptyHistoryBody, icon: Icons.history_rounded)
                        : _HistoryList(items: hist.history, offline: settings.offlineMode, dataMode: settings.dataMode)),
              ],
            ),
          ),
        ],
...
class _FavoritesList extends StatelessWidget {
  const _FavoritesList({required this.items, required this.offline, required this.dataMode});
      ...
      itemBuilder: (context, i) {
        ...
        return Dismissible(
          ...
          child: _RouteTile(
            ...
            onRun: () async {
              context.read<AnalyticsService>().logEvent('saved_rerun', params: {'type': 'favorite', 'modes': f.modes.map((e) => e.storageKey).join(',')});
              await context.read<FareController>().search(origin: f.origin, destination: f.destination, modes: f.modes, offline: offline, dataMode: dataMode);
              context.go('/search');
            },
            trailing: Row(
  ...
  String _modesLabel(BuildContext context, List<TransportMode> modes) {
    final l10n = TffLocalizations.of(context);
    final parts = modes.map((m) => switch (m) { TransportMode.hsr => l10n.modesHSR, TransportMode.tra => l10n.modesTRA, TransportMode.mrt => l10n.modesMRT, TransportMode.bus => l10n.modesBus, TransportMode.youBike => l10n.modesYouBike }).toList();
    return parts.join(' • ');
  }
}

class _HistoryList extends StatelessWidget {
  const _HistoryList({required this.items, required this.offline, required this.dataMode});
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Expanded(child: Text(l10n.history, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700))),
              TffTextButton(
                label: l10n.clearAll,
            ...
            itemBuilder: (context, i) {
              ...
              return Dismissible(
                ...
                child: _RouteTile(
                  ...
                  onRun: () async {
                    context.read<AnalyticsService>().logEvent('saved_rerun', params: {'type': 'history', 'modes': h.modes.map((e) => e.storageKey).join(',')});
                    await context.read<FareController>().search(origin: h.origin, destination: h.destination, modes: h.modes, offline: offline, dataMode: dataMode);
...
class _RouteTile extends StatelessWidget {
  const _RouteTile({required this.title, required this.subtitle, required this.onRun, required this.trailing});
      ...
      child: TffCard(
        ...
        child: Row(
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(subtitle, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: cs.onSurfaceVariant), maxLines: 1, overflow: TextOverflow.ellipsis),
                ],
              ),
            ),
            ...
            trailing,
            const SizedBox(width: AppSpacing.sm),
            TffTextButton(label: l10n.rerun, icon: Icons.play_arrow_rounded, onPressed: onRun),
          ],
        ),
      ),
    );
...
Future<bool?> _confirmClearHistory(BuildContext context) async {
  ...
  return showDialog<bool>(
    ...
    builder: (context) {
      return AlertDialog(
        title: Text(l10n.clearHistory, maxLines: 2, overflow: TextOverflow.ellipsis),
...
class _SwipeDeleteBg extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: cs.errorContainer.withValues(alpha: 0.85), borderRadius: BorderRadius.circular(AppRadius.xl)),
